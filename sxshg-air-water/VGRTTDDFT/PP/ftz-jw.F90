...
Program ft
  ...
  real(8) :: hw,tt,dt,Eamp(3),domega,avg(1),dAc,pi,tfac,tdamp
  complex(8) :: jav_w(1),E_ext_w(1),E_tot_w(1)
  complex(8) :: zsigma_w(1),zeps(1),zI
  real(8),allocatable ::javt(:,:),tz(:)
...
!damp and smooth
  ...
  do iter=1,Nt
     tt=(iter-0.5)*dt
     javt(iter,:)= javt(iter,:)*exp(-tt/tdamp)*smoothing_t(tt,dt,Nt)
  enddo
...
!  javt = javt/13.60581d0   !convert to a.u?
  ...
  do ihw=1,Nomega
     ...
     do iter=1,Nt
        tt=(iter-0.5)*dt
        jav_w(:)=jav_w(:)+javt(iter,:)*exp(zI*hw*tt)!*smoothing_t(tt,dt,Nt)!*exp(-tt/tdamp)
     enddo
     jav_w(:)=jav_w(:)*dt; 
